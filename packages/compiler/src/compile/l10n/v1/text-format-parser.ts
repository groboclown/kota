...
import { HasErrorValue, coreError } from '@kota/base-libs'
import {
  ArgumentMapping,
  ArgumentReference,
  UnformattedTextFragment,
 ...
 * parsing.
 * 
 * This does not perform style handling.  That is done in a seperate process
 * after this function call, by inspecting the plain message formatting, and
 * needing to correctly spread that to the child items.
 */
export function parseTextFormat(format: string): UnformattedTextFragment[] | HasErrorValue {
  // Stack of the
